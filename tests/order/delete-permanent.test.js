...
// npx jest tests/order/delete-permanent.test.js

describe("DELETE /api/orders/:orderId/permanent", () => {
  let adminEmail;
  let adminPassword;
  ...
  it("delete permanent order should be successful", async () => {
  ...
  it("update permanent order should fail if role is not admin", async () => {
    const loginRes = await supertest(app).post("/api/users/login").send({
      email: userEmail,
