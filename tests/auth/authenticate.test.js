...
// npx jest tests/auth/authenticate.test.js
...
describe("POST /api/users/authenticate", () => {
  ...
  it("authenticate user should fail without accessToken cookie", async () => {
    const result = await supertest(app).post("/api/users/authenticate").send({
      password: userPassword,
    });
